...
const commented = await page.evaluate(() => (window.__trove.app.social.get().sidecar?.comments || []).length);
...
await page.waitForFunction(() => (window.__trove.app.search.get().paletteFiles || []).length > 0, { timeout: 10000 }).catch(() => {});
...
await page.evaluate(() => window.__trove.platform.commands.execute('workbench.view.plugins'));
...
await page.evaluate(() => {
  ...
  const node = t.app.explorer.get().items.find((i) => i.name === 'clip.demo');
  const plugin = t.platform.contributions.ofType('opener').find((o) => o.pluginId);
  t.test.open(node, plugin.id);
});
